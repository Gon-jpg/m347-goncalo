...
```bash
docker create -p 8082:80 --name my-nginx nginx
```
...
- `-p 8082:80` mappt Port 8082 des Hosts auf Port 80 des Containers
...
Falls der Port 8082 bereits belegt ist, muss der andere Container zuerst gestoppt werden:
...
Nach dem Start ist die Nginx-Willkommensseite unter `http://localhost:8082` erreichbar.
